...
import myTest.testConcurrent.exception.BusinessException;
import myTest.testConcurrent.exception.ExceptionLauncher;
import myTest.testConcurrent.exception.ExceptionLauncherImpl;
...
public class TestCompletableFuture {
    ...
    /**
     * 注意,这里模拟的是"网络请求"! 所以多线程那边,主线程在阻塞等待时,必须调用有timeout的API! 否则容易造成很多线程僵死,导致最后服务不可用!!!
     *
     * @return
     */
    ...
    public static List<String> test2() throws BusinessException {
        ...
        //要拿这个返回值! 虽然方法里面已经用API和上面的list完成了数据的聚拢,但是,如果里面调用的业务方法中,处理的是网络请求,那么,主线程必须调用有timeout的阻塞方法,防止对方网络接口出问题而导致本地服务线程的僵死.
